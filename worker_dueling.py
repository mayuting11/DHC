...
class GlobalBuffer:
    def __init__(self, episode_capacity=configs_D3QTP.episode_capacity,
        ...
        self.obs_buf = np.zeros(
            ((local_buffer_capacity + 1) * episode_capacity, configs_D3QTP.max_num_agents, *configs_D3QTP.obs_shape), dtype=bool)
        ...
        self.hid_buf = np.zeros((local_buffer_capacity * episode_capacity, configs_D3QTP.max_num_agents, configs_D3QTP.hidden_dim),
                                dtype=np.float16)
    ...
    def stats(self, interval: int):
        ...
        for num_agents in range(configs_D3QTP.init_env_settings[0], configs_D3QTP.max_num_agents + 1):
            ...
            if (num_agents, configs_D3QTP.max_map_lenght, configs_D3QTP.max_map_width) in self.stat_dict:
                print('{:<3d}'.format(len(self.stat_dict[(num_agents, configs_D3QTP.max_map_lenght, configs_D3QTP.max_map_width)])), end='')
            else:
                print('   N/A  ', end='')
            print()
        ...
        for key, val in self.stat_dict.copy().items():
            ...
            if len(val) == 200:
                ...
                add_agent_key = (key[0] + 1, key[1], key[2])
    ...
    def check_done(self):
        for i in range(configs_D3QTP.max_num_agents):
            if (i + 1, configs_D3QTP.max_map_lenght, configs_D3QTP.max_map_width) not in self.stat_dict:
                return False

            l = self.stat_dict[(i + 1, configs_D3QTP.max_map_lenght, configs_D3QTP.max_map_width)]
    ...
    def train(self):
        ...
        while not ray.get(self.buffer.check_done.remote()) and self.counter < configs_D3QTP.training_times:
            ...
            for i in range(1, 10001):
                ...
                b_q = self.model(b_obs[:, :-configs_D3QTP.forward_steps], b_obs_agent, b_seq_len, b_hidden).gather(1, b_action)
                ...
                if i % configs_D3QTP.save_interval == 0:
                    torch.save(self.model.state_dict(), os.path.join(configs_D3QTP.save_path, '{}.pth'.format(self.counter)))
...
class Actor:
    ...
    def reset(self):
        ...
        local_buffer = LocalBuffer(self.id, self.env.num_agents, self.env.map_size[1], self.env.map_size[0], obs,  obs_agent)
